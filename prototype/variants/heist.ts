...
/* ================================================================
   ...
   Layer map (all scenery kill-switched via --heist-scenery):
     ...
     head::before             laser tripwire #2 — a second red beam
                              guarding the right margin between the door
                              and dossier. STATIC (coarse, soft, off-lane)
...
html::after {
  ...
  transform: translateZ(0);
  background:
    /* center-lane readability scrim (topmost in this stack) — coarse, soft;
       fades to 0 well before the right edge so the vault door keeps its
       steel. Lives here (not on body::after) so body::after is free for the
       dial telltale, which must paint ABOVE the door. */
...
/* the head void-elements render nothing but their fixed pseudos are
   free canvases (L7 kill-switch on head itself too). */
head { display: var(--heist-scenery, block); }
head meta { display: var(--heist-scenery, block); }

/* ═══ laser tripwire #2 — a second red beam guarding the RIGHT margin,
   pinned as a near-vertical bar between the dossier corner and the vault
   door so the two tripwires bracket the stage from opposite sides. Now
   with an emitter pip at its upper origin and a soft floor-hit glow where
   it meets the door frame, plus a haze halo. Drawn as a rotated gradient
   BOX confined to the right edge, clear of both the reading column and the
   door steel. Coarse, soft. STATIC, promoted. ═══ */
head::before {
  content: "";
  display: var(--heist-scenery, block);
  position: fixed;
  ...
  transform: translateZ(0);
  background:
    /* emitter pip at the top of beam #2 */
    radial-gradient(circle 6px at 88vw 4vh, rgba(255, 240, 232, 0.9), rgba(255, 96, 86, 0.65) 42%, rgba(255, 59, 48, 0) 78%),
    /* soft glow where beam #2 grazes the door frame lower-right */
    radial-gradient(ellipse 8vw 6vh at 95vw 52vh, rgba(255, 90, 78, 0.22), rgba(255, 59, 48, 0) 74%),
    /* the beam core */
    linear-gradient(102deg, rgba(255, 59, 48, 0) 43%, rgba(255, 59, 48, 0.1) 45%, rgba(255, 118, 108, 0.5) 48% 52%, rgba(255, 59, 48, 0.1) 55%, rgba(255, 59, 48, 0) 57%) 90vw -8vh / 22vw 58vh no-repeat,
    /* haze halo hugging beam #2 */
    linear-gradient(102deg, rgba(255, 70, 60, 0) 40%, rgba(255, 70, 60, 0.06) 50%, rgba(255, 70, 60, 0) 60%) 90vw -8vh / 22vw 58vh no-repeat;
}
